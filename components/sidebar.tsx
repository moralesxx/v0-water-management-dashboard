...
import { 
  LayoutDashboard, Users, Home, CreditCard, Droplets, 
  GitBranch, AlertTriangle, LogOut, Shield, Wallet, Zap 
} from "lucide-react"
...
export function Sidebar({ currentView, onNavigate, user, onLogout }: SidebarProps) {
  ...
  const esAdministrativo = roleClean === "ADMIN" || roleClean === "TESORERO" || roleClean === "ENCARGADO";

  return (
    <aside className="w-64 bg-sidebar text-sidebar-foreground border-r border-sidebar-border flex flex-col">
      <div className="p-6 border-b border-sidebar-border">
        <div className="flex items-center justify-center">
          {esAdministrativo ? <IconoMenuAdmin /> : <IconoMenu />}
        </div>
      </div>
      ...
      <div className="p-4 border-t border-sidebar-border space-y-3">
        <div className="flex items-center gap-3 px-3 py-2">
          <div className={cn("w-8 h-8 rounded-full flex items-center justify-center text-primary-foreground", roleInfo.color)}>
        ...
        <Button variant="ghost" size="sm" onClick={onLogout} className="w-full justify-start gap-2 text-sidebar-foreground/70 hover:text-sidebar-foreground hover:bg-sidebar-accent/50">
