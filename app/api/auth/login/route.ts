...
export async function POST(req: NextRequest) {
  try {
   ...
   // ... Todo tu código anterior de la API se queda EXACTAMENTE IGUAL

    // 🟢 Modificamos solo el objeto de respuesta para asegurar compatibilidad con tu Sidebar
    const response = NextResponse.json({
      ok: true,
      usuario: {
        id: usuario.id,
        nombre: usuario.nombre,
        username: usuario.nombre, // 🟢 Satisface a user.username en el Sidebar
        email: usuario.email,
        rol: usuario.rol,
        role: usuario.rol,         // 🟢 Satisface a user.role en el Sidebar
      },
    });
